...
export class PersonCreate extends Component {
    ...
    PersonUpdateSchema = yup.object().shape({
        userName: yup.string().max(50).min(1).required("required").label('User Name'),
        name: yup.string().max(50).min(1).required().label('Name'),
    ...
    render(){
        return <Formik
        initialValues={{isActive:true, startDate: '', userName: '', name: ''}}
